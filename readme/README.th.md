...
**แปลทั้งไฟล์ เลย์เอาต์อยู่ครบ** เซิร์ฟเวอร์ MCP สำหรับ [Equalang](https://equalang.com) - เครื่องมือแปลภาษาด้วย AI ที่ทำงานกับไฟล์ทั้งไฟล์: ส่ง PDF ไปก็ได้ PDF กลับมา ส่งสไลด์ไปก็ได้สไลด์กลับมา ตาราง รูปภาพ และสูตรอยู่ที่เดิมครบ นอกจากนี้ยังแปลคำบรรยายและรูปภาพ เปลี่ยนเสียงและวิดีโอเป็นคำบรรยายที่แปลแล้วหรือบทถอดเสียง และแปลข้อความสั้น ๆ จำนวนมากในคราวเดียว ใช้ได้ใน Claude Code, Claude Desktop, Codex, Cursor, Windsurf, Cline, VS Code และไคลเอนต์ MCP อื่นทุกตัว
...
- “แปล ~/Documents/contract.pdf เป็นภาษาไทย โดยคงเลย์เอาต์เดิมไว้”
- “แปล pitch-deck.pptx เป็นภาษาอังกฤษและภาษาจีน”
- “แปล https://example.com/whitepaper.pdf เป็นภาษาไทย แล้วบันทึกไว้ที่ ~/Downloads”
- “ถ้าแปล thesis.docx เป็นภาษาอังกฤษ จะเสียเท่าไร”
- “ทำซับไตเติลภาษาไทยให้ interview.mp4 โดยให้บรรทัดต้นฉบับอยู่เหนือคำแปลทุกบรรทัด”
- “ถอดเสียง standup.m4a เป็นข้อความพร้อมเวลากำกับ”
- “แปลข้อความใน menu.jpg เป็นภาษาไทย”
- “แปลข้อความใน locales/en.json เป็นภาษาญี่ปุ่น เกาหลี และเวียดนาม”

## ฟีเจอร์

- **เอกสาร** - PDF, DOCX, PPTX, XLSX, EPUB, HTML และ TXT ได้กลับมาในรูปแบบเดิม ยังแก้ไขต่อได้ ตาราง รูปภาพ สูตร และเลย์เอาต์ของหน้าอยู่ครบ
...
- **ข้อความจำนวนมาก** - ข้อความสั้น ๆ หลายรายการถูกแปลตามลำดับ หรือข้อความยาวหนึ่งข้อความ (สูงสุด 100,000 อักขระ) ที่ Equalang ตัดตามประโยคเอง
- **ภาษา** - กว่า 100 ภาษาสำหรับข้อความ และ 12 ภาษาสำหรับไฟล์ โดยตรวจจับภาษาต้นทางให้เองเมื่อไม่ได้ระบุ
...
คีย์ใส่ไว้ในตัวแปรสภาพแวดล้อมในคอนฟิกของไคลเอนต์ MCP ไม่ใส่ใน URL เด็ดขาด ถึงไม่มีคีย์ เซิร์ฟเวอร์ก็ยังเริ่มทำงานและแสดงรายการเครื่องมือได้ ส่วนเครื่องมือที่ต้องใช้คีย์จะตอบกลับด้วยวิธีขอคีย์

หรือจะเก็บคีย์ไว้เครื่องละครั้งใน `~/.config/equalang/.env` ก็ได้ ซึ่งสกิลของ Equalang ก็อ่านไฟล์นี้เช่นกัน:
...
เซิร์ฟเวอร์จะดู `EQUALANG_API_KEY` ในตัวแปรสภาพแวดล้อมของตัวเองก่อน ถ้าไม่มีจึงอ่านไฟล์นี้ คีย์ที่ใส่ในคอนฟิกของไคลเอนต์จึงมีผลก่อน และถ้ามีไฟล์นี้แล้ว คอนฟิกของไคลเอนต์ก็ไม่ต้องมี `env`
...
| `translate_text` | แปลข้อความสั้น ๆ หลายรายการ ตามลำดับ - หรือข้อความยาวหนึ่งข้อความ ซึ่ง Equalang จะตัดตามประโยคเอง |
...
| `list_languages` | รหัสและชื่อภาษาทั้งหมด ไม่ต้องใช้คีย์ |

รหัสภาษามีหน้าตาแบบ `en`, `zh-CN`, `ja` และ `list_languages` มีรายการทั้งหมด งานใช้เวลาหลายนาที - เครื่องมือจะรอได้ไม่เกิน `wait_seconds` (ค่าเริ่มต้น 50 s, สูงสุด 240) แล้วคืน id ของงานไว้ให้ `check_job` มารับต่อ
