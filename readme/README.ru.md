...
- «Переведи на русский текст с menu.jpg.»
- «Переведи строки из locales/en.json на немецкий, французский и испанский.»
...
Ключ можно также сохранить один раз на компьютер в `~/.config/equalang/.env` в виде `EQUALANG_API_KEY=el_your_key`: сервер читает этот файл, когда в его окружении ключа нет, и скилл Equalang тоже — тогда в конфигурации клиента `env` не нужен.
