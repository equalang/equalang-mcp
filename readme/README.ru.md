...
Коды языков выглядят так: `en`, `zh-CN`, `ja`; `list_languages` читает их из работающего API, поэтому язык, добавленный в Equalang, доступен без обновления. Задания занимают минуты — инструмент ждёт не дольше `wait_seconds` (по умолчанию 50 с, максимум 240), после чего возвращает id задания для `check_job`.
