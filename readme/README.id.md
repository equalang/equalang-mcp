...
Kode bahasa berbentuk seperti `en`, `zh-CN`, `ja`; `list_languages` membacanya dari API secara langsung, sehingga bahasa yang ditambahkan Equalang tidak memerlukan pembaruan di sini. Job memakan waktu beberapa menit - tool menunggu hingga `wait_seconds` (bawaan 50 s, paling lama 240), lalu mengembalikan id job untuk `check_job`.
