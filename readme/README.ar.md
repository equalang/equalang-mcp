...
تبدو رموز اللغات هكذا: `en` و`zh-CN` و`ja`؛ و`list_languages` تقرأها من الـAPI الحي، فأي لغة يضيفها Equalang لا تحتاج إلى تحديث هنا. المهام تستغرق دقائق - تنتظر الأداة حتى `wait_seconds` (50 s افتراضيًا، و240 على الأكثر)، ثم تعيد معرّف مهمة لـ`check_job`.
