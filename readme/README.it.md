...
**Traduci il file, conserva l'impaginazione.** Un server MCP per [Equalang](https://equalang.com), un traduttore AI che lavora su file interi: un PDF torna come PDF, una presentazione come presentazione, con tabelle, immagini e formule dov'erano. Traduce anche sottotitoli e immagini, trasforma audio e video in sottotitoli tradotti o in una trascrizione, e traduce testi brevi in blocco. Funziona in Claude Code, Claude Desktop, Codex, Cursor, Windsurf, Cline, VS Code e in ogni altro client MCP.
...
La chiave va in una variabile d'ambiente nella configurazione del client MCP, mai in un URL. Senza chiave il server si avvia comunque ed elenca i suoi strumenti; uno strumento che ha bisogno della chiave risponde spiegando come ottenerla.

La chiave può anche stare una volta sola per macchina, in `~/.config/equalang/.env`, che legge anche la skill di Equalang:

```bash
# Sostituisci el_your_key con la tua chiave
mkdir -p ~/.config/equalang && echo 'EQUALANG_API_KEY=el_your_key' > ~/.config/equalang/.env && chmod 600 ~/.config/equalang/.env
```

Il server prende prima `EQUALANG_API_KEY` dal suo ambiente e legge il file solo se lì non c'è: una chiave nella configurazione del client ha la precedenza, e con il file la configurazione del client non ha bisogno di `env`.
...
| `translate_text` | Traduce testi brevi, nell'ordine dato – oppure un unico testo lungo, che Equalang divide da sé in frasi. |
...
| `list_languages` | Codici e nomi di tutte le lingue. Non richiede chiave. |

I codici delle lingue hanno la forma `en`, `zh-CN`, `ja`; l'elenco completo è in `list_languages`. I job durano minuti – uno strumento aspetta fino a `wait_seconds` (50 s per impostazione predefinita, 240 al massimo), poi restituisce un id del job che `check_job` può riprendere.
